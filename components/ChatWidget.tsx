...
const ChatWidget: React.FC<ChatWidgetProps> = ({ clientId }) => {
  ...
  useEffect(() => {
    const fetchConfig = async () => {
      ...
      else console.warn('Using default config, Supabase record not found.');
    };
    fetchConfig();
    ...
    <div className="fixed bottom-8 right-8 z-[2147483647] font-sans selection:bg-indigo-100">
      {!isOpen ? (
        ...
        <div className="bg-white w-[420px] h-[720px] rounded-[3rem] shadow-[0_50px_100px_-20px_rgba(0,0,0,0.4)] flex flex-col overflow-hidden border border-slate-200/50 animate-in fade-in zoom-in-95 slide-in-from-bottom-10 duration-500">
          <div className="p-10 text-white relative overflow-hidden" style={{ backgroundColor: config.primary_color }}>
            ...
            <div className="relative z-10 flex justify-between items-center">
              <div className="flex items-center space-x-4">
                ...
                <div>
                  <h3 className="font-black text-xl tracking-tight leading-none mb-1.5">{config.name}</h3>
            ...
            {showLeadForm && (
              <div className="bg-white p-10 rounded-[2.5rem] border border-slate-100 shadow-2xl space-y-6 animate-in zoom-in-95 ring-1 ring-slate-100">
                ...
                <form onSubmit={submitLead} className="space-y-3">
                  ...
                  <button type="submit" className="w-full text-white py-5 rounded-2xl font-black text-sm shadow-xl hover:scale-[1.02] transition-all active:scale-95" style={{ backgroundColor: config.primary_color }}>
                    Apply for Access
                  </button>
                </form>
              </div>
            ...
            {isTyping && (
              <div className="flex items-center space-x-3 text-slate-400">
                ...
                <span className="text-[9px] font-black uppercase tracking-widest">
                  {config.thinking_enabled ? 'Synthesizing Intelligence' : 'Processing'}
                </span>
              </div>
            )}
          </div>
